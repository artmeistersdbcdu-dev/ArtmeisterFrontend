...
if(!artWorks){
    return (
        <div className="col-span-1 md:col-span-2 flex flex-col items-center justify-center py-16 text-center border border-overlay/5 rounded-2xl bg-overlay/5 border-dashed space-y-3">
            <div className="p-4 rounded-full bg-overlay/5 text-muted-foreground">
              <ImageIcon className="w-8 h-8" />
            </div>
            <div>
              <p className="text-muted-foreground font-medium text-lg">All caught up!</p>
              <p className="text-muted-foreground text-sm max-w-xs mx-auto">No pending artworks to review at the moment.</p>
            </div>
          </div>
    );
}
  return (
    <div className="space-y-8 w-full max-w-5xl mx-auto animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="px-4 py-2 rounded-xl border border-overlay/10 bg-overlay/5 backdrop-blur-md text-sm text-muted-foreground font-medium">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-start gap-2">
                  ...
                  <a
                    ...
                    className="text-muted-foreground hover:text-content p-1 rounded-md hover:bg-overlay/5 transition-colors"
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {artItem.Tags.map((tag, idx) => (
                      <Badge
                        key={idx}
                        variant="secondary"
                        className="bg-overlay/5 text-muted-foreground border-overlay/5 text-[10px] font-normal px-2 py-0"
                      >
